...
6. The serializer must handle address choice: BRA → `<endNac>` (cMun + CEP), non-BRA → `<endExt>` (cPais + cEndPost + xCidade + xEstProvReg — always emitted, callers must provide valid non-empty values per XSD minLength), followed by xLgr, nro (S/N fallback), xCpl (conditional), xBairro. Simple address (TCEnderecoSimples) for obra/atvEvento: CEP or endExt.
...
8. The serializer must handle values (TCInfoValores) with vServPrest, vDescCondIncond (conditional), vDedRed (pDR or vDR or documentos with TCDocDedRed list — choice key chNFSe/chNFe/NFSeMun/NFNFS/nDocFisc/nDoc + tpDedRed + dtEmiDoc + vDedutivelRedutivel + vDeducaoReducao + fornec optional), trib containing tribMun (tribISSQN mapping, cPaisResult, tpImunidade, exigSusp, BM, tpRetISSQN, pAliq), tribFed (piscofins with CST/vBCPisCofins/aliquots/vPis/vCofins/tpRetPisCofins XSD v1.01 values 1-2, vRetCP, vRetIRRF, vRetCSLL), totTrib (Simples → pTotTribSN, amounts → vTotTrib, rates → pTotTrib, indTotTrib → "0" when NotInformed, fallback → zeroed vTotTrib).
9. The serializer must generate `<IBSCBS>` (TCRTCInfoIBSCBS) complete when IbsCbs.ClassCode is not null, containing: finNFSe (always "0"), indFinal ("0"/"1"), cIndOp (6 digits), tpOper (optional), gRefNFSe (optional, max 99 refNFSe), tpEnteGov (optional), indDest ("0"/"1"), dest (optional — TCRTCInfoDest with CNPJ/CPF/NIF/NaoNIF + xNome + end), imovel (optional — inscImobFisc + choice cCIB/end), valores (required — gReeRepRes with reimbursement documents + trib→gIBSCBS with CST + cClassTrib + gTribRegular optional + gDif optional with pDifUF, pDifMun, pDifCBS when deferment data present).
10. The mapper must convert all fields from `NfseGenerateXmlRequest` to `DpsDocument`, including intermediary, location, values (fiscal), and all optional groups (ForeignTrade, Lease, Construction, ActivityEvent, AdditionalInformationGroup, Deduction, Benefit, Suspension, ApproximateTotals, IbsCbs). The IbsCbs mapping must convert the typed `IbsCbsRequest` DTO to the domain `IbsCbs` model including Purpose, PersonalUse, OperationIndicator, ClassCode, SituationCode, DestinationIndicator, RelatedDocs, GovernmentPurchase, RegularTaxation, ThirdPartyReimbursements (with documents, suppliers, dates, types), Recipient (Person), and RealEstate.
11. The endpoint POST `/api/v1/nfse/xml` must use `NationalDpsManualSerializer` to generate XML.
12. The project must have unit tests for the mapper (including IBSCBS scenarios: minimal, with recipient, with reimbursements, null) and integration tests for the endpoint (including request with IBSCBS validating XML output contains `<IBSCBS>` block) using WebApplicationFactory.

13. The project must have golden master XML snapshots (minimal-dps.xml, complete-dps.xml) in tests/Snapshots/ for regression comparison via XNode.DeepEquals, validated against XSD.
