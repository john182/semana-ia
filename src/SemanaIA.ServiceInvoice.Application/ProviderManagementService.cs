...
public class ProviderManagementService
{
        ...
        string? primaryXsdFile = null)
    {
        ...
        // Auto-gerar regras default e profile a partir do XSD
        var autoGeneratedRules = AutoGenerateRules(provider);
        var generatedProfile = autoGeneratedRules.Profile;
        generatedProfile.Rules = autoGeneratedRules.Rules;

        // Always save the profile — even without rules, it contains RootComplexTypeName,
        // RootElementName, BindingPathPrefix, and other metadata needed for serialization.
        var rulesJson = System.Text.Json.JsonSerializer.Serialize(generatedProfile);
        provider.UpdateRulesJson(rulesJson);

        if (generatedProfile.RootComplexTypeName is not null)
