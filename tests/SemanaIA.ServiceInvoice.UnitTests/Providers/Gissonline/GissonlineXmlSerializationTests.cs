...
namespace SemanaIA.ServiceInvoice.UnitTests.Providers.Gissonline;
...
public class GissonlineXmlSerializationTests
{
    ...
    public void Given_FillingVariation_Should_ProduceSchemaValidXml(string scenarioName, DpsDocument document)
    {
        ...
        // Assert
        result.Xml.ShouldNotBeNull($"[{scenarioName}] Serialization failed: {FormatErrors(result)}");
        result.Errors.ShouldBeEmpty($"[{scenarioName}] Errors: {FormatErrors(result)}");
